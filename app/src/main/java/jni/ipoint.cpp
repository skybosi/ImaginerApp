...
PIXELS::PIXELS():pix_X(-1),pix_Y(-1),rgb_threshold(128),bEdge(0),pix_P(None,0)
{
    memset(&prgb,0,sizeof(RGBQUAD));
    //rgb_threshold  = 0;
}

PIXELS::PIXELS(const RGBQUAD& rgb):pix_X(-1),pix_Y(-1),rgb_threshold(128),bEdge(0),pix_P(None,0)
{
...
{
    PIXELS ppot3;
    ppot3.prgb.rgbRed = (((ppot1.prgb.rgbRed * weight) + (ppot2.prgb.rgbRed *(1 - weight))) >> 2)/100;
    ppot3.prgb.rgbGreen = (((ppot1.prgb.rgbGreen * weight) + (ppot2.prgb.rgbGreen *(1 - weight))) >> 2)/100;
    ppot3.prgb.rgbBlue = (((ppot1.prgb.rgbBlue * weight) + (ppot2.prgb.rgbBlue *(1 - weight))) >> 2)/100;
    ppot3.prgb.rgbReserved = (((ppot1.prgb.rgbReserved * weight) + (ppot2.prgb.rgbReserved *(1 - weight))) >> 2)/100;
    return ppot3;
}
...
//set rgb with r g b
PIXELS PIXELS::setRGB(U8 b,U8 g,U8 r,U8 a)
...
void PIXELS::setpPosStatus(Position pos)
{
    pix_P.first = pos;
    pix_P.second |= 0x80;
}
...
PIXELS PIXELS::operator+(const RGBQUAD& rgb)
{
    PIXELS add;
    add.prgb.rgbRed = prgb.rgbRed + rgb.rgbRed;
    add.prgb.rgbGreen = prgb.rgbGreen + rgb.rgbGreen;
    add.prgb.rgbBlue = prgb.rgbBlue + rgb.rgbBlue;
    add.prgb.rgbReserved = prgb.rgbReserved + rgb.rgbReserved;
    return add;
}

//just add of two pixel
const PIXELS operator+(const RGBQUAD& rgb,const PIXELS& pixel)
{
    PIXELS add;
    add.prgb.rgbRed = rgb.rgbRed + pixel.prgb.rgbRed;
    add.prgb.rgbGreen = rgb.rgbGreen + pixel.prgb.rgbGreen;
    add.prgb.rgbBlue = rgb.rgbBlue + pixel.prgb.rgbBlue;
    add.prgb.rgbReserved = rgb.rgbReserved + pixel.prgb.rgbReserved;
    return add;
}
